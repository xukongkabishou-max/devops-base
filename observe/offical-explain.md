...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/instance: release-name
    app.kubernetes.io/version: "75.15.0"
    app.kubernetes.io/part-of: kube-prometheus-stack
    chart: kube-prometheus-stack-75.15.0
...
- target_matchers:
  - alertname = InfoInhibitor
receivers:
- name: web.hook.promalert.feishu
  webhook_configs:
  - send_resolved: false
    url: http://prometheus-alert-center:8080/prometheusalert?type=fs&tpl=fs-tpl-pretty&fsurl=https://open.feishu.cn/open-apis/bot/v2/hook/9161cc69-5893-4277-96ce-f94876e47aa7&split=false
- name: web.hook.promalert.email
  webhook_configs:
  - send_resolved: true
    url: http://prometheus-alert-center:8080/prometheusalert?type=email&tpl=email-tpl-pretty&email=wangshihong@ecmas.ai,caosikai@ecmas.ai,wangjian@ecmas.ai,zhutianyu@ecmas.ai&split=false
- name: "null"
route:
  group_by:
  - namespace
...
这是一个 Kubernetes ConfigMap，里面存储了 Grafana 的 Dashboard Provider 配置文件 provider.yaml。

providers 定义了一个名为 sidecarProvider 的仪表盘提供者（provider）。

type: file 表示 Grafana 会从文件系统中的指定路径加载仪表盘 JSON 文件。

path: /tmp/dashboards 指定了仪表盘文件所在的目录，Grafana 会定期（每 30 秒）扫描这个目录，自动加载或更新仪表盘。

disableDeletion: false 表示允许删除仪表盘。

allowUiUpdates: false 表示禁止用户通过 UI 修改这些仪表盘，避免破坏自动管理的仪表盘。
...
data:
  datasource.yaml: |-
    ...
    - name: "Prometheus"
      type: prometheus
      uid: prometheus
      url: http://release-name-kube-promethe-prometheus.observe:9090/
      access: proxy
      isDefault: true
      jsonData:
        httpMethod: POST
    ...
    - name: "Alertmanager"
      type: alertmanager
      uid: alertmanager
      url: http://release-name-kube-promethe-alertmanager.observe:9093/
      access: proxy
      jsonData:
    ...
    - access: proxy
      isDefault: false
      jsonData:
        maxLines: 1000  //最大返回数据1000行
      name: loki
      type: loki
      uid: loki
      url: http://release-name-kube-promethe-loki.observe:3100
---
...
spec:
  ...
  automountServiceAccountToken: true
  externalUrl: http://release-name-kube-promethe-alertmanager.observe:9093
  paused: false
  logFormat: "logfmt"
  logLevel:  "info"
  retention: "120h"
        ...
        podAffinityTerm:
          ...
          labelSelector:
            matchExpressions:
              ...
              - {key: alertmanager, operator: In, values: [release-name-kube-promethe-alertmanager]}
  portName: http-web
...
spec:
  ...
  imagePullPolicy: "IfNotPresent"
  version: v3.5.0
  externalUrl: http://release-name-kube-promethe-prometheus.observe:9090
  paused: false
  replicas: 1
  shards: 1
  logLevel:  info
      ...
      - weight: 100
        podAffinityTerm:
          ...
          labelSelector:
            ...
            - {key: app.kubernetes.io/instance, operator: In, values: [release-name-kube-promethe-prometheus]}
  portName: http-web
  hostNetwork: false
---
# Source: kube-prometheus-stack/templates/prometheus/rules-1.14/alertmanager.rules.yaml

你理解得很对，这段 PrometheusRule 定义的是一组针对 Alertmanager 状态和行为的告警规则，用于监控 Alertmanager 是否正常工作，并在出现异常时发出告警。

具体来说，这些告警涵盖了：

AlertmanagerFailedReload
配置重载失败，提示 Alertmanager 配置文件加载出错。

AlertmanagerMembersInconsistent
集群成员不一致，某个 Alertmanager 实例没有发现所有其他成员，提示集群通信或成员发现异常。

AlertmanagerFailedToSendAlerts
某个实例通知发送失败比例超过阈值，提示通知发送异常。

AlertmanagerClusterFailedToSendAlerts
整个 Alertmanager 集群向某些通知集成失败，分为“关键”和“非关键”集成。

AlertmanagerConfigInconsistent
集群中各 Alertmanager 实例配置不一致，可能导致行为不统一。

AlertmanagerClusterDown
集群中至少一半的实例在最近 5 分钟内不可用。

AlertmanagerClusterCrashlooping
集群中至少一半的实例在最近 10 分钟内频繁重启（CrashLoop）。

apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-alertmanager.rules
  namespace: observe
  labels:
  ...
  - name: alertmanager.rules
    ...
    - alert: AlertmanagerFailedReload
      ...
      expr: |-
        ...
        max_over_time(alertmanager_config_last_reload_successful{job="release-name-kube-promethe-alertmanager",namespace="observe"}[5m]) == 0
      for: 10m
      labels:
        severity: critical
    - alert: AlertmanagerMembersInconsistent
      ...
      expr: |-
        # Without max_over_time, failed scrapes could create false negatives, see
        # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
          max_over_time(alertmanager_cluster_members{job="release-name-kube-promethe-alertmanager",namespace="observe"}[5m])
        < on (namespace,service,cluster) group_left
          count by (namespace,service,cluster) (max_over_time(alertmanager_cluster_members{job="release-name-kube-promethe-alertmanager",namespace="observe"}[5m]))
      for: 15m
      labels:
        severity: critical
    - alert: AlertmanagerFailedToSendAlerts
      ...
      expr: |-
        (
          rate(alertmanager_notifications_failed_total{job="release-name-kube-promethe-alertmanager",namespace="observe"}[15m])
        /
          ignoring (reason) group_left rate(alertmanager_notifications_total{job="release-name-kube-promethe-alertmanager",namespace="observe"}[15m])
        )
        > 0.01
      for: 5m
      labels:
    ...
    - alert: AlertmanagerClusterFailedToSendAlerts
      ...
      expr: |-
        min by (namespace,service, integration) (
          rate(alertmanager_notifications_failed_total{job="release-name-kube-promethe-alertmanager",namespace="observe", integration=~`.*`}[15m])
        /
          ignoring (reason) group_left rate(alertmanager_notifications_total{job="release-name-kube-promethe-alertmanager",namespace="observe", integration=~`.*`}[15m])
        )
        > 0.01
      for: 5m
      labels:
    ...
    - alert: AlertmanagerConfigInconsistent
      ...
      expr: |-
        count by (namespace,service,cluster) (
          count_values by (namespace,service,cluster) ("config_hash", alertmanager_config_hash{job="release-name-kube-promethe-alertmanager",namespace="observe"})
        )
        != 1
      for: 20m
      labels:
    ...
    - alert: AlertmanagerClusterDown
      ...
      expr: |-
        (
          count by (namespace,service,cluster) (
            avg_over_time(up{job="release-name-kube-promethe-alertmanager",namespace="observe"}[5m]) < 0.5
          )
        /
          count by (namespace,service,cluster) (
            up{job="release-name-kube-promethe-alertmanager",namespace="observe"}
          )
        )
        >= 0.5
      for: 5m
    ...
    - alert: AlertmanagerClusterCrashlooping
      ...
      expr: |-
        (
          count by (namespace,service,cluster) (
            changes(process_start_time_seconds{job="release-name-kube-promethe-alertmanager",namespace="observe"}[10m]) > 4
          )
        /
          count by (namespace,service,cluster) (
            up{job="release-name-kube-promethe-alertmanager",namespace="observe"}
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-etcd
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.container-cpu-usage-second
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.container-memory-cache
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.container-memory-rss
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.container-memory-swap
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.container-memory-working-s
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.container-resource
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-k8s.rules.pod-owner
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kube-apiserver-burnrate.rules
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kube-apiserver-slos
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kube-scheduler.rules
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-apps
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-resources
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-storage
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-system-apiserver
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-system-kube-proxy
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-system-kubelet
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-system-scheduler
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-kubernetes-system
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
kind: PrometheusRule
metadata:
  name: release-name-kube-promethe-prometheus-operator
  namespace: observe
  labels:
    app: kube-prometheus-stack
...
spec:
  ...
  - name: prometheus-operator
    ...
    - alert: PrometheusOperatorListErrors
      ...
      expr: (sum by (cluster,controller,namespace) (rate(prometheus_operator_list_operations_failed_total{job="release-name-kube-promethe-operator",namespace="observe"}[10m])) / sum by (cluster,controller,namespace) (rate(prometheus_operator_list_operations_total{job="release-name-kube-promethe-operator",namespace="observe"}[10m]))) > 0.4
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusOperatorWatchErrors
      ...
      expr: (sum by (cluster,controller,namespace) (rate(prometheus_operator_watch_operations_failed_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m])) / sum by (cluster,controller,namespace) (rate(prometheus_operator_watch_operations_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m]))) > 0.4
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusOperatorSyncFailed
      ...
      expr: min_over_time(prometheus_operator_syncs{status="failed",job="release-name-kube-promethe-operator",namespace="observe"}[5m]) > 0
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusOperatorReconcileErrors
      ...
      expr: (sum by (cluster,controller,namespace) (rate(prometheus_operator_reconcile_errors_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m]))) / (sum by (cluster,controller,namespace) (rate(prometheus_operator_reconcile_operations_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m]))) > 0.1
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusOperatorStatusUpdateErrors
      ...
      expr: (sum by (cluster,controller,namespace) (rate(prometheus_operator_status_update_errors_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m]))) / (sum by (cluster,controller,namespace) (rate(prometheus_operator_status_update_operations_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m]))) > 0.1
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusOperatorNodeLookupErrors
      ...
      expr: rate(prometheus_operator_node_address_lookup_errors_total{job="release-name-kube-promethe-operator",namespace="observe"}[5m]) > 0.1
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusOperatorNotReady
      ...
      expr: min by (cluster,controller,namespace) (max_over_time(prometheus_operator_ready{job="release-name-kube-promethe-operator",namespace="observe"}[5m]) == 0)
      for: 5m
      labels:
        severity: warning
    - alert: PrometheusOperatorRejectedResources
      ...
      expr: min_over_time(prometheus_operator_managed_resources{state="rejected",job="release-name-kube-promethe-operator",namespace="observe"}[5m]) > 0
      for: 5m
      labels:
        severity: warning
---
  ...
  - name: prometheus
    ...
    - alert: PrometheusBadConfig
      ...
      expr: |-
        # Without max_over_time, failed scrapes could create false negatives, see
        # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
        max_over_time(prometheus_config_last_reload_successful{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) == 0
      for: 10m
      labels:
        severity: critical
    - alert: PrometheusSDRefreshFailure
      ...
      expr: increase(prometheus_sd_refresh_failures_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[10m]) > 0
      for: 20m
      labels:
        severity: warning
    - alert: PrometheusKubernetesListWatchFailures
      ...
      expr: increase(prometheus_sd_kubernetes_failures_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusNotificationQueueRunningFull
      ...
      expr: |-
        ...
        # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
        (
          predict_linear(prometheus_notifications_queue_length{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m], 60 * 30)
        >
          min_over_time(prometheus_notifications_queue_capacity{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        )
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusErrorSendingAlertsToSomeAlertmanagers
      ...
      expr: |-
        (
          rate(prometheus_notifications_errors_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        /
          rate(prometheus_notifications_sent_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        )
        * 100
        > 1
      for: 15m
    ...
    - alert: PrometheusNotConnectedToAlertmanagers
      ...
      expr: |-
        # Without max_over_time, failed scrapes could create false negatives, see
        # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
        max_over_time(prometheus_notifications_alertmanagers_discovered{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) < 1
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusTSDBReloadsFailing
      ...
      expr: increase(prometheus_tsdb_reloads_failures_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[3h]) > 0
      for: 4h
      labels:
        severity: warning
    - alert: PrometheusTSDBCompactionsFailing
      ...
      expr: increase(prometheus_tsdb_compactions_failed_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[3h]) > 0
      for: 4h
      labels:
        severity: warning
    - alert: PrometheusNotIngestingSamples
      ...
      expr: |-
        (
          sum without(type) (rate(prometheus_tsdb_head_samples_appended_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])) <= 0
        and
          (
            sum without(scrape_job) (prometheus_target_metadata_cache_entries{job="release-name-kube-promethe-prometheus",namespace="observe"}) > 0
          or
            sum without(rule_group) (prometheus_rule_group_rules{job="release-name-kube-promethe-prometheus",namespace="observe"}) > 0
          )
        )
      for: 10m
      labels:
    ...
    - alert: PrometheusDuplicateTimestamps
      ...
      expr: rate(prometheus_target_scrapes_sample_duplicate_timestamp_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusOutOfOrderTimestamps
      ...
      expr: rate(prometheus_target_scrapes_sample_out_of_order_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 10m
      labels:
        severity: warning
    - alert: PrometheusRemoteStorageFailures
      ...
      expr: |-
        (
          (rate(prometheus_remote_storage_failed_samples_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) or rate(prometheus_remote_storage_samples_failed_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]))
        /
          (
            (rate(prometheus_remote_storage_failed_samples_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) or rate(prometheus_remote_storage_samples_failed_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]))
          +
            (rate(prometheus_remote_storage_succeeded_samples_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) or rate(prometheus_remote_storage_samples_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]))
          )
        )
        * 100
        > 1
    ...
    - alert: PrometheusRemoteWriteBehind
      ...
      expr: |-
        ...
        # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
        (
          max_over_time(prometheus_remote_storage_highest_timestamp_in_seconds{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        - ignoring(remote_name, url) group_right
          max_over_time(prometheus_remote_storage_queue_highest_sent_timestamp_seconds{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        )
        > 120
      for: 15m
      labels:
    ...
    - alert: PrometheusRemoteWriteDesiredShards
      annotations:
        description: Prometheus {{$labels.namespace}}/{{$labels.pod}} remote write desired shards calculation wants to run {{ $value }} shards for queue {{ $labels.remote_name}}:{{ $labels.url }}, which is more than the max of {{ printf `prometheus_remote_storage_shards_max{instance="%s",job="release-name-kube-promethe-prometheus",namespace="observe"}` $labels.instance | query | first | value }}.
      ...
      expr: |-
        ...
        # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
        (
          max_over_time(prometheus_remote_storage_shards_desired{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        >
          max_over_time(prometheus_remote_storage_shards_max{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m])
        )
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusRuleFailures
      ...
      expr: increase(prometheus_rule_evaluation_failures_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: critical
    - alert: PrometheusMissingRuleEvaluations
      ...
      expr: increase(prometheus_rule_group_iterations_missed_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusTargetLimitHit
      ...
      expr: increase(prometheus_target_scrape_pool_exceeded_target_limit_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusLabelLimitHit
      ...
      expr: increase(prometheus_target_scrape_pool_exceeded_label_limits_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusScrapeBodySizeLimitHit
      ...
      expr: increase(prometheus_target_scrapes_exceeded_body_size_limit_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusScrapeSampleLimitHit
      ...
      expr: increase(prometheus_target_scrapes_exceeded_sample_limit_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusTargetSyncFailure
      ...
      expr: increase(prometheus_target_sync_failed_total{job="release-name-kube-promethe-prometheus",namespace="observe"}[30m]) > 0
      for: 5m
      labels:
        severity: critical
    - alert: PrometheusHighQueryLoad
      ...
      expr: avg_over_time(prometheus_engine_queries{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) / max_over_time(prometheus_engine_queries_concurrent_max{job="release-name-kube-promethe-prometheus",namespace="observe"}[5m]) > 0.8
      for: 15m
      labels:
        severity: warning
    - alert: PrometheusErrorSendingAlertsToAnyAlertmanager
      ...
      expr: |-
        min without (alertmanager) (
          rate(prometheus_notifications_errors_total{job="release-name-kube-promethe-prometheus",namespace="observe",alertmanager!~``}[5m])
        /
          rate(prometheus_notifications_sent_total{job="release-name-kube-promethe-prometheus",namespace="observe",alertmanager!~``}[5m])
        )
        * 100
        > 3
      for: 15m
